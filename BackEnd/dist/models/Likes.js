...
router.put("/:id", (req, res) => {
    ...
    pool.query("UPDATE likes SET liked = $1 WHERE id = $2", [req.body.liked, id], (error, results) => {
        if (error) {
            throw error;
        }
...
router.get("/:id", (req, res) => {
    ...
    pool.query("SELECT * FROM likes WHERE user_id = $1 AND liked = true", [userId], (error, results) => {
        if (error) {
            throw error;
        }
        else if (results.rows.length === 0) {
...
function addLike(recipe_id, user_id, liked, res) {
    pool.query("INSERT INTO likes (recipe_id, user_id, liked) VALUES ($1, $2, $3) RETURNING *", [recipe_id, user_id, liked], (error, results) => {
        if (error) {
            throw error;
        }
...
function updateLike(recipe_id, user_id, liked, res) {
    pool.query("UPDATE likes SET liked = $1 WHERE recipe_id = $2 AND user_id = $3 RETURNING *", [liked, recipe_id, user_id], (error, results) => {
        if (error) {
            throw error;
        }
...
router.delete("/", (req, res) => {
    pool.query("DELETE FROM likes", (error, results) => {
        if (error) {
            throw error;
        }
