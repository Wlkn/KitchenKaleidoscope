...
router.post("/", (req, res) => {
    ...
    pool.query("SELECT to_regclass('comments')", (error, results) => {
        if (error) {
            throw error;
        }
        ...
        pool.query("INSERT INTO comments (recipe_id, user_id, comment) VALUES ($1, $2, $3) RETURNING *", [recipe_id, user_id, comment], (error, results) => {
            if (error) {
                throw error;
            }
            res.status(201).json(`Comment added with ID: ${results.rows[0].id}`);
...
router.put("/:id", (req, res) => {
    ...
    pool.query("UPDATE comments SET comment = $1 WHERE id = $2", [req.body.comment, id], (error, results) => {
        if (error) {
            throw error;
        }
...
function createTables() {
    pool.query("CREATE TABLE comments (id SERIAL PRIMARY KEY, recipe_id CHAR(24) , user_id CHAR(24), comment TEXT NOT NULL, created_at TIMESTAMP DEFAULT NOW())", (error) => {
        if (error) {
            throw error;
        }
